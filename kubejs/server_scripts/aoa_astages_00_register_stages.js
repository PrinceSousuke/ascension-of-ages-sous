...
// Registers each AoA stage with AStages via customizeStage() (permanent registry /
// /astages add validation) plus a no-op marker restriction on minecraft:barrier.
// Barrier is unobtainable in survival, so the restriction is invisible to players.
// This silences the astages-common.toml "Enable Warning" for any stage with no
// real item lock - notably the ren_* milestone flags, which are pure progression markers.
...
;(function () {
  ...
  function registerStage(stage) {
    // customizeStage registers the stage in AStages' permanent manager and ALL_STAGES.
    // Marker restrictions alone can miss command autocomplete / add validation on some builds.
    try {
