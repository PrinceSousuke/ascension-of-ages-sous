...
- This file is a reference map, not source-of-truth architecture.
- `Oritech` spans Industrial Revolution through Ascension, with apex/cosmic use later.
- `Modern Industrialization` is a major Industrial/Gilded process-industry branch.
- `Atomic` is the required containment/consequence bridge between Gilded and Otherworldly.
- `Mekanism` and related hazardous-power / strategic-tech content belong in Atomic first, with late handoff only after containment proofs.
- `Stellaris` is installed and remains an Otherworldly launch candidate.
- `NauTec` belongs in Gilded oceanic industry unless the Required Chapter Matrix changes.
- Existing `Minecolonies` and structure content remain annex/support, not required catalog content.

## Architecture Authority

Use `age_domain_boss_architecture.md` as subordinate reference material for:
...
Use `late_game_redesign_structure.md` as subordinate reference material for:

- Industrial -> Atomic -> Space age boundaries
...
- Renaissance outward support: installed magic/ritual systems, Apotheosis, Artifacts, Relics, AE2, Refined Storage (optional), Nether/realm clusters. NauTec is Gilded.
...
- Otherworldly/Ascension convergence: Stellaris, Draconic, Extended Crafting, Re:Avaritia, final archive synthesis
...
| Entering the Medieval | Minecraft iron threshold, stage grant | AoA current `entering_the_iron_era` | Live gate now checks Stone survival-readiness capstones plus stable travel supplies, without leaking iron early |
| Dark Age Boss Hunts | early bosses and validators | boss export only | AoA-original |
...
| Renaissance Aether Spine | Aether entry + Slider + Deep Aether Eye of the Storm; Sun Spirit/Gold Dungeon/Gravitite/Phoenix Bow are Gilded Aether culmination/depth, and Valkyrie Queen remains reserved for IR | live `ren_aether_literacy`, boss proof contract, Aether jar evidence, Gilded staging docs | Aether is authored bounded realm progression, not an optional destination branch; Sun Spirit is not Renaissance requirement space |
...
| Aether Required Realm Spine | controlled entry, route prep, Slider, Deep Aether Eye of the Storm; Valkyrie Queen moves to IR and Sun Spirit/Gold Dungeon/Gravitite/Phoenix Bow move to Gilded Aether culmination/depth | live `ren_aether_literacy`, AOF `the_aether`, boss proof contract | Aether is a required bounded realm lane; Sun Spirit is not Renaissance requirement space |
...
| Luminous Nether Hunts | removed-mod tombstone only | live mods folder + removed-mod cleanup reports | No Luminous Nether or Luminous:Tag jar is installed; do not author this branch unless the mod is deliberately reinstalled and reverified |
...
| Aether and Deep Aether | Aether, Deep Aether | AOF `the_aether` | Required bounded realm-boss spine |
...
| Blaze Breakthrough | nether/blaze threshold and branch proof | AoA current `blaze_breakthrough`, progression exports, and AoA gate policy | Blaze now remains a mandatory Nether branch proof and workshop-enabler, while the Industrial Revolution stage grant belongs to the Nether Gauntlet conquest surface |
| Renaissance Realm Boss Hunts | Aether, Undergarden, Warden-tier validators | boss export + AoA redo authority | Aether branch is required spine; other realm-depth validators follow current boss contract |
...
| Industrial Revolution Boss Hunts | machine-age combat validators and Industrial-opening capstone | boss exports + age-domain architecture | Live Automation boss spine. It now turns End-ready factory ownership into the real Industrial Age gate through Obsidilith, Ancient Factory routing, Ender Golem, and Ender Guardian |
...
| Dragonfall and End Access | industrial readiness proof, End access, Dragonfall, outer-End follow-through, Ancient Factory routing, The Harbinger | progression exports + Cataclysm advancement chain | Live Industrial -> Atomic gate. Mature factory branches now feed a real conquest bridge that grants `atomic` only after post-dragon scale and The Harbinger are defeated |

## Atomic Age

| AoA Chapter | Live Systems | Primary References | Notes |
| --- | --- | --- | --- |
| Containment Protocols | hazard discipline, shielding mindset, sealed-facility setup | `late_game_redesign_structure.md` | Live Atomic opener now correctly sits behind `atomic` and uses hazmat gear, shielding, scrubbers, waste handling, and the first Nuclear Science extraction/separation tools |
...
| Ascension Final Exam | Chaos Guardian / Archive final thesis only; earlier bosses stay distributed age exams | boss exports, progression design bible, 2026-06-01 boss-ladder correction | AoA-original |
